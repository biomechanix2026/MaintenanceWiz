...
- **LLM mode** (`run_llm` in `agent/orchestrator.py`) — a single Codex tool-use loop with the full tool set. This is the "Consolidated Brain": one loop, no sub-agents.
...
| Reasoning | Codex tool-use loop | deterministic Python pipeline | `agent/orchestrator.py` |
